...
void RenderFoam(uint waveIdx, uint idx, float time, float maxFrames, float exposure)
{
	...
	lightColor = lerp(sceneColor * lightColor, lightColor, data.ao);

	half3 fogColor;
	half3 fogOpacity;
	float3 viewDir = GetWorldSpaceViewDirNorm(worldPos);
	GetInternalFogVariables(float4(screenUV * _FoamRTSize.xy, foamDepth, 1), viewDir, LinearEyeDepth(foamDepth), screenPos.z, fogColor, fogOpacity);
